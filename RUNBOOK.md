...
#### The GUI walk-through — three clicks

This is the version to film. Set the state up first:

```bash
ssh im-builder 'cd ~/rhel-lightwell && ./scripts/trustify-flow.sh gui'
```

Then open `http://192.168.122.195:8080` and click through the left-hand menu —
do not type URLs, the UI drops the route on a hard load.

**1. SBOMs.** Two rows, `1.1` and `1.2`, 3664 dependencies each — and read the
**Vulnerabilities** column:

| Version | Dependencies | Vulnerabilities |
|---|---|---|
| 1.1 | 3664 | **4** |
| 1.2 | 3664 | **0** |

> "Two SBOMs, generated by syft from the two images I actually pushed. Same
> application, 3664 components each, differing by exactly one. Four
> vulnerabilities on the build from this morning. Zero on the one I shipped
> twenty seconds ago."

This is the single best screen in the act — **don't rush past it.** It only
reads this way because the backport is complete and the VEX is loaded; with a
partial fix, or with the public feed also ingested, 1.2 does not reach zero.

**2. Vulnerabilities.** Read the **Impacted SBOMs** column: **1, 1, 1, 1**.

> "Four CVEs, and every one of them now impacts a single build — the one I
> haven't patched. Including the 7.8."

**3. CVE-2024-56326 → Related SBOMs.** Two rows: `1.1 Affected`, `1.2 Fixed`.
